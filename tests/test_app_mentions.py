...
class AppMentionTests(unittest.TestCase):
    ...
    def test_app_boot_registers_app_mention_handler_without_redis_url(self):
        fake_app_instance = MagicMock()
        ...
        with patch.dict(
            ...
            "config.CONFIG",
            {"SLACK_BOT_TOKEN": "xoxb-test", "SLACK_APP_TOKEN": "xapp-test", "REDIS_URL": ""},
            clear=False,
        ):
            module = importlib.import_module("app")
            module = importlib.reload(module)
        ...
        self.assertIn("app_mention", event_handlers)
        fake_redis_module.Redis.from_url.assert_not_called()
